...
export default function QuizDEA() {
  ...
  const setWrongQuestions = (updater) => {
    setWrongQuestionsState((prev) => {
      ...
      window.storage.set("wrongQuestions", JSON.stringify(next), false).catch(() => {});
      return next;
    });
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("officialResults", false);
    ...
    (async () => {
      try {
        const result = await window.storage.get("wrongQuestions", false);
    ...
    (async () => {
      try {
        const result = await window.storage.get("seenQuestions", false);
  ...
  const saveOfficialResults = async (list) => {
    ...
    try {
      await window.storage.set("officialResults", JSON.stringify(list), false);
    } catch (e) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("quizHistory", false);
  ...
  const saveHistory = async (newHistory) => {
    ...
    try {
      await window.storage.set("quizHistory", JSON.stringify(newHistory), false);
    } catch (e) {
  ...
  const markSeen = (qids) => {
    setSeenQuestionsState((prev) => {
      ...
      window.storage.set("seenQuestions", JSON.stringify(next), false).catch(() => {});
      return next;
    });
  };

  const startQuiz = (year, review = false) => {
    const limit = quizLength === "Todas" ? null : parseInt(quizLength);
    const qs = review ? wrongQuestions : buildQuestions(year, filterArea, shuffle, onlyUnseen, limit);
    if (qs.length === 0) {
  ...
  const clearHistory = async () => {
    try {
      await window.storage.delete("quizHistory", false);
    } catch (e) {
      // nada que borrar
    }
    setHistory([]);
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {mode === "exam" && <span style={{ color: "#f59e0b", fontSize: 13, fontWeight: 700, fontFamily: "monospace" }}>⏱️ {fmt(examTime)}</span>}
